@forelse ($enumerators as $enumerator)
    <tr>
        ...
        <td style="font-size:12.5px;color:var(--adm-text-mid);">
            {{ $enumerator->koordinator->nama_lengkap ?? '-' }}
        </td>
        <td>
        ...
        <td class="tc">
            <div class="adm-actions" style="justify-content:center;flex-wrap:wrap;">
                @if (!$enumerator->user_id)
                    <button type="button"
                        class="adm-btn warning btn-generate-user"
                        data-id="{{ $enumerator->id }}"
                        data-nama="{{ $enumerator->nama_lengkap }}"
                        data-hp="{{ $enumerator->telephone }}"
                        title="Generate akun user">
                        <svg viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>
                        User
                    </button>
                @endif
                <a class="adm-btn icon-only" href="{{ route('superadmin.enumerators.gallery', $enumerator->hashed_id) }}" title="Galeri">
                    <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                </a>
                <a class="adm-btn primary icon-only" href="{{ route('superadmin.enumerators.show', $enumerator->hashed_id) }}" title="Detail">
                    <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                </a>
                <a class="adm-btn success icon-only" href="{{ route('superadmin.enumerators.edit', $enumerator->hashed_id) }}" title="Edit">
                    <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </a>
                <button type="button" class="adm-btn danger icon-only btn-delete" data-id="{{ $enumerator->id }}" title="Hapus">
